...
export const MemberList = () => {
  ...
  const { setBoardActive, boardActive, userActive, setUserActive } = useStore(
    store,
    (s) => s
  );
  ...
  const handleActive = async (miembro: IMiembro, index: number) => {
    ...
    const { data } = await getBoard(
      boardActive.id,
      miembro.id == userActive.id ? null : miembro.id
    );
    setBoardActive({
      nombre: boardActive.nombre,
      id: boardActive.id,
  ...
  return (
    <>
      {miembros?.map((miembro, index) => (
        <div className="p-3 w-80" key={miembro.id} role="button">
          <FormCheck
