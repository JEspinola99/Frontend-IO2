...
export const Kanban = () => {
  ...
  const { boardActive, loadingKanban, setBoardActive } = useStore(
    store,
    (s) => s
  );
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const activeCol = boardActive.columnas.find(
      (col) => col.id == active?.data?.current?.parent
    );
    const activeColIndex = boardActive.columnas.findIndex(
      (col) => col.id == active?.data?.current?.parent
    );
    const overColIndex = boardActive.columnas.findIndex(
      (col) => col.id == over?.id
    );

    const updatedItems = activeCol?.tareas.filter(
      (item) => item.id != active?.data?.current?.id
    ) as ITask[];
    const removedItem = activeCol?.tareas.filter(
      (item) => item.id == active?.data?.current?.id
    ) as ITask[];
    try {
          ...
          >
            <div className="grid grid-flow-col gap-3 justify-start">
              {boardActive.columnas?.map((container) => (
                <div className="" key={container.id}>
                  <ContainerColumn
